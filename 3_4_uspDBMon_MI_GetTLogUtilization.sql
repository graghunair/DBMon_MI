...
DROP PROCEDURE IF EXISTS [dbo].[uspDBMon_MI_GetTLogUtilization]
GO

CREATE PROCEDURE [dbo].[uspDBMon_MI_GetTLogUtilization]
AS
	/*
...
IF EXISTS (SELECT TOP 1 1 FROM [dbo].[tblDBMon_SP_Version] WHERE [SP_Name] = 'uspDBMon_MI_GetTLogUtilization')
	BEGIN
		...
		WHERE	[SP_Name] = 'uspDBMon_MI_GetTLogUtilization'
	END
ELSE
	BEGIN
...
EXEC [dbo].[uspDBMon_MI_TrackDBAChanges] 'Installed SP: uspDBMon_MI_GetTLogUtilization'
GO
