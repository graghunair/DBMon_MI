...
DROP PROCEDURE IF EXISTS [dbo].[uspDBMon_MI_GetTableSize]
GO

CREATE proc [dbo].[uspDBMon_MI_GetTableSize]
...
AS
	/*
		...
		This script is provided "AS IS" with no warranties, and confers no rights.
					EXEC [dbo].[uspDBMon_MI_GetTableSize] @Database_Name = 'dba_local'
...
IF EXISTS (SELECT TOP 1 1 FROM [dbo].[tblDBMon_SP_Version] WHERE [SP_Name] = 'uspDBMon_MI_GetTableSize')
	BEGIN
		UPDATE	[dbo].[tblDBMon_SP_Version]
		SET		[SP_Version] = '1.0 GTS',
				[Last_Executed] = NULL,
				[Date_Modified] = GETDATE(),
				[Modified_By] = SUSER_SNAME()
		WHERE	[SP_Name] = 'uspDBMon_MI_GetTableSize'
	END
ELSE
	BEGIN
		INSERT INTO [dbo].[tblDBMon_SP_Version] ([SP_Name], [SP_Version], [Last_Executed], [Date_Modified], [Modified_By])
		VALUES ('uspDBMon_MI_GetTableSize', '1.0 GTS', NULL, GETDATE(), SUSER_SNAME())
	END
GO
...
EXEC sp_addextendedproperty 
	...
	@level1type = 'PROCEDURE', @level1name = 'uspDBMon_MI_GetTableSize'
GO

EXEC [dbo].[uspDBMon_MI_TrackDBAChanges] 'Installed SP: uspDBMon_MI_GetTableSize'
GO